<?xml version="1.0" encoding="UTF-8"?>
<tileset version="1.10" tiledversion="1.11.2" name="titleset" tilewidth="16" tileheight="16" spacing="2" margin="2" tilecount="672" columns="32">
 <image source="../../../Tilemap_color3-removebg-preview.png" width="576" height="384"/>
 <tile id="298">
  <animation>
   <frame tileid="33" duration="100"/>
   <frame tileid="34" duration="100"/>
   <frame tileid="35" duration="100"/>
   <frame tileid="36" duration="100"/>
   <frame tileid="37" duration="100"/>
  </animation>
 </tile>
 <tile id="329">
  <objectgroup draworder="index" id="4">
   <object id="3" x="4" y="1.75" visible="0">
    <polygon points="0,0 11,1 -1,5"/>
   </object>
  </objectgroup>
 </tile>
 <tile id="550">
  <animation>
   <frame tileid="33" duration="300"/>
   <frame tileid="34" duration="300"/>
   <frame tileid="35" duration="300"/>
   <frame tileid="36" duration="300"/>
   <frame tileid="37" duration="300"/>
   <frame tileid="38" duration="300"/>
   <frame tileid="39" duration="300"/>
  </animation>
 </tile>
 <wangsets>
  <wangset name="Unnamed Set" type="corner" tile="-1">
   <wangcolor name="" color="#ff0000" tile="-1" probability="1"/>
   <wangcolor name="" color="#00ff00" tile="-1" probability="1"/>
   <wangtile tileid="308" wangid="0,1,0,2,0,1,0,1"/>
   <wangtile tileid="309" wangid="0,1,0,1,0,2,0,1"/>
   <wangtile tileid="340" wangid="0,2,0,1,0,1,0,1"/>
   <wangtile tileid="341" wangid="0,1,0,1,0,1,0,2"/>
   <wangtile tileid="374" wangid="0,2,0,1,0,2,0,2"/>
   <wangtile tileid="375" wangid="0,2,0,2,0,1,0,2"/>
   <wangtile tileid="406" wangid="0,1,0,2,0,2,0,2"/>
   <wangtile tileid="407" wangid="0,2,0,2,0,2,0,1"/>
  </wangset>
 </wangsets>
</tileset>
